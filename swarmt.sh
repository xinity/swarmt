...
swarm_init(){
    ...
    export worker_token=$(docker-machine ssh ${project}m1 "docker swarm \
    join-token worker -q")

    export manager_token=$(docker-machine ssh ${project}m1 "docker swarm \
...
main() {
  ...
  then
      ...
      if [ "$1" != "-c" -a -s ${cfg_file} -a -r ${cfg_file} ]
      then
          echo "$1"
          source ${cfg_file}
      fi
